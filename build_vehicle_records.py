...
def main(_):
    ...
    for (dType, inputPath, outputPath) in datasets:
        ...
        for image in soup.find_all('image'):
            ...
            #tags = re.findall(r'<image file={}>(.*?)<//image>'.format(image["file"]), html, re.DOTALL)
            #txt1 = "<image file='{}'>".format(image["file"])
            #print(txt1)
            #tags = re.findall(r'<image file={}>(.*?)<\\image>'.format(image["file"]), contents, re.DOTALL)
            tags = re.findall(r'<image file=\''+image["file"]+'\'>(.*?)</image>'.format(image["file"]), contents, re.DOTALL)
            #soup1 = image.find_all(attrs='box ')
            soup1 = BeautifulSoup(str(tags), 'html.parser')
            for box in soup1.find_all('box'):
                #par = box.findParent()
                #par1 = par['image']
